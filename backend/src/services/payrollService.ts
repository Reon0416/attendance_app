...
): { normalMinutes: number; lateNightMinutes: number } {
  let normalMinutes = 0;
...
export function calculatePayroll(
  ...
  user: { hourlyRate: number; lateNightRate: number }
) {
  ...
  let workSegments: { start: Date; end: Date }[] = [];
      ...
      case "CLOCK_OUT":
        if (clockInTime && lastActiveTime) {
          ...
          for (const segment of workSegments) {
            const { normalMinutes, lateNightMinutes } = calculateTimeSegments(
              segment.start,
              segment.end
            );
