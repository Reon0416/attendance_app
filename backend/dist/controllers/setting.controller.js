 ...
 */
export async function updateUserIdHandler(req, res) {
    const user = req.user;
 ...
 */
export async function updateRateHandler(req, res) {
    const user = req.user;
    const { hourlyRate, lateNightRate } = req.body;
    if (user.role !== "OWNER") {
        return res
            .status(403)
 ...
 */
export async function accountRegisterHandler(req, res) {
    const user = req.user;
