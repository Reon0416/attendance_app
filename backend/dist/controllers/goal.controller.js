 ...
 */
export async function setGoalHandler(req, res) {
    const user = req.user;
    ...
    try {
        await prisma.$transaction(async (tx) => {
            ...
            if (oldActiveGoal) {
                ...
                if (!rateRecord)
                    throw new Error("時給情報が未設定です。");
                const attendanceRecords = await getAttendanceRecordsSince(user.id, oldActiveGoal.createdAt);
                const payrollResult = calculatePayroll(attendanceRecords, rateRecord);
...
const needsMonthlyReset = (goalCreatedAt, now) => {
    return (goalCreatedAt.getMonth() !== now.getMonth() ||
 ...
 * 月初めの目標リセットも同時に処理する。
 */
export async function getGoalProgressHandler(req, res) {
    const user = req.user;
    if (!user) {
        return res.status(401).json({ message: "未認証です" });
    }
    try {
        ...
        const attendanceRecords = await getAttendanceRecordsSince(user.id, activeGoal.createdAt);
        // 目標設定後の給与を計算
        const payrollResult = calculatePayroll(attendanceRecords, rateRecord);
        const earnedAmount = payrollResult.totalPay;
